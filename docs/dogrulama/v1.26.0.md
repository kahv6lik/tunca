# Doğrulama Raporu — v1.26.0

| | |
|---|---|
| **Sonuç** | **✅ GEÇTİ** |
| Geçen kontrol | 886 |
| Kalan kontrol | 0 |
| Sürüm | `v1.26.0` |
| Commit | `65bacd8` |
| Dal | `claude/crm-structure-versioning-naarbc` |
| Tarih | 2026-08-12 08:45 |

> Bu rapor `npm run dogrula` ile otomatik üretildi. Doğrulama, temiz bir
> veritabanı üzerinde sıfırdan kurulum yaparak çalışır; geliştirme
> veritabanına dokunmaz.

## Özet

| Kontrol | Sonuç |
|---------|-------|
| ✅ TypeScript tip kontrolü | hata yok |
| ✅ Üretim derlemesi | başarılı |
| ✅ Migration'lar temiz veritabanına uygulandı | 20 migration |
| ✅ İki kiracılı demo veri üretildi | 11 adım |
| ✅ Otomatik testler (Vitest) | 569/569 kontrol |
| ✅ Port 3100 boş | eski sunucu kalıntısı yok |
| ✅ Sunucu ayağa kalktı | http://localhost:3100 |
| ✅ Cevap veren sunucu bu çalıştırmaya ait | pid=27207 |
| ✅ Kiracı izolasyonu — HTTP | 14/14 kontrol |
| ✅ Kimlik doğrulama — gerçek tarayıcı | 296/296 kontrol |

## Ayrıntılar

### Otomatik testler (Vitest)

```

```

### Kiracı izolasyonu — HTTP

```
1. Liste sayıları kiracıya göre
  ✅ A kendi firma sayısını görüyor — 800 = 800
  ✅ B kendi firma sayısını görüyor — 120 = 120
2. Aktif kuruluş göstergesi
  ✅ A kendi kuruluşunu görüyor
  ✅ A, B'nin kuruluş adını görmüyor
  ✅ B kendi kuruluşunu görüyor
3. Doğrudan URL ile çapraz kiracı erişimi
  ✅ A, B'nin firma verisini göremiyor — "Holding Tarım San. Tic. 1"
  ✅ A'ya 404 sayfası gösteriliyor
  ✅ B'nin vergi numarası sızmıyor
  ✅ A, B'nin düzenleme formunu açamıyor
  ✅ B kendi firmasını normal görüyor
4. Listelerde sızıntı
  ✅ B'nin firması A'nın listesinde yok
  ✅ B'nin yatırım kaydı A'nın listesinde yok
5. Oturum kontrolleri
  ✅ Girişsiz erişim /login'e yönleniyor — HTTP 307
  ✅ tenantId taşımayan oturum reddediliyor — HTTP 307
```

### Kimlik doğrulama — gerçek tarayıcı

```
1. Demo yönetici hesabı
  ✅ Demo hesap veritabanında var — admin@gezegen.com
  ✅ Demo hesabın rolü kuruluş yöneticisi — rol=tenant_admin
  ✅ Demo hesabın kiracısı aktif — durum=aktif
  ✅ Demo şifresi 'admin123' geçerli
2. Giriş formu (gerçek tarayıcı)
  ✅ Doğru bilgiyle giriş başarılı — http://localhost:3100/
  ✅ Panele yönlendirildi
  ✅ Kendi kuruluşunu görüyor
  ✅ İkinci kiracı da giriş yapabiliyor — http://localhost:3100/
  ✅ İkinci kiracı kendi kuruluşunu görüyor
  ✅ İkinci kiracı diğerinin adını görmüyor
3. Hatalı giriş reddediliyor
  ✅ Yanlış şifre reddedildi
  ✅ Genel hata mesajı gösteriliyor
  ✅ Var olmayan e-posta reddedildi
  ✅ Hata mesajı kullanıcının varlığını sızdırmıyor — yanlış şifre ile aynı mesaj
4. Kiracılar arası kimlik sızıntısı
  ✅ Bir kiracının şifresi diğerinde işe yaramıyor
5. Askıya alınmış kiracı
  ✅ Askıya alınmış kiracının kullanıcısı giremiyor
  ✅ Açıklayıcı mesaj gösteriliyor
  ✅ Yeniden aktifleştirilince girebiliyor
6. Yetkilendirme — roller farklı şey görüyor
  ✅ Yönetici menüde 'Ayarlar' görüyor
  ✅ Yönetici menüde 'Denetim Günlüğü' görüyor
  ✅ Yönetici rolü üst çubukta yazıyor
  ✅ Yönetici gruplar sayfasını açabiliyor
  ✅ Yönetici denetim günlüğünü açabiliyor
  ✅ Üye menüde 'Gruplar' GÖRMÜYOR
  ✅ Üye menüde 'Denetim Günlüğü' GÖRMÜYOR
  ✅ Üye rolü üst çubukta yazıyor
  ✅ Üye doğrudan URL ile gruplara giremiyor — http://localhost:3100/yetkisiz?izin=grup.yonet
  ✅ Üye doğrudan URL ile denetim günlüğüne giremiyor
  ✅ Salt okunur firma listesini görebiliyor
  ✅ Salt okunur rolü üst çubukta yazıyor
  ✅ Grup üyeliği salt okunur kullanıcıya 'Yeni Firma' yetkisi ekliyor — grup: Saha Ekibi → firma.olustur
  ✅ Salt okunur raporları görebiliyor
7. Admin panel — platform sınırı
  ✅ Platform yöneticisi hesabı var ve rolü doğru — rol=platform_admin
  ✅ Platform yöneticisi admin paneli açabiliyor
  ✅ Admin panelde bütün kuruluşlar görünüyor — kiracılar ötesi görünüm yalnızca burada
  ✅ Platform yöneticisi paketleri yönetebiliyor
  ✅ Kuruluş yöneticisi admin paneline giremiyor — http://localhost:3100/yetkisiz?izin=kiraci.yonet
  ✅ Kuruluş yöneticisi başka kuruluşun adını görmüyor
  ✅ Üye admin paneline giremiyor — http://localhost:3100/yetkisiz?izin=kiraci.yonet
  ✅ Davet sayfası girişsiz açılıyor — http://localhost:3100/davet/gecersiz-token-denemesi
  ✅ Geçersiz davet reddediliyor
  ✅ Geçersiz davette hiçbir kuruluş adı sızmıyor
8. Satış çekirdeği — kişiler, fırsatlar, kanban
  ✅ Yönetici kontak listesini açabiliyor
  ✅ Yönetici satış hattını açabiliyor
  ✅ Kanban aşama sütunları görünüyor — varsayılan hat: Yeni → İletişim → Teklif → Müzakere → Sonuç
  ✅ Hat özeti hesaplanıyor
  ✅ Yönetici aşamaları yönetebiliyor
  ✅ Üye aşama yönetimine giremiyor — http://localhost:3100/yetkisiz?izin=firsat.asama
  ✅ Üye fırsatları görebiliyor
  ✅ Üye menüde 'Aşamalar' düğmesini GÖRMÜYOR
  ✅ Salt okunur fırsatları görebiliyor
  ✅ Salt okunur 'Yeni Fırsat' düğmesini GÖRMÜYOR
  ✅ İkinci kiracı kendi hattını görüyor
  ✅ İkinci kiracı diğerinin firmasını fırsatlarda görmüyor
9. Satış derinleştirme — aktivite, aday, teklif, timeline
  ✅ Yönetici aktiviteleri açabiliyor
  ✅ Bugün / Açık Görevler / Akış sekmeleri var
  ✅ Yönetici adayları açabiliyor
  ✅ Aday hunisi ve dönüşüm oranı hesaplanıyor
  ✅ Yönetici teklifleri açabiliyor
  ✅ Teklif durum özeti görünüyor
  ✅ Firma detayında zaman akışı var
  ✅ Akışta farklı modüllerden kayıtlar birleşiyor
  ✅ Salt okunur adayları görebiliyor
  ✅ Salt okunur 'Yeni Aday' düğmesini GÖRMÜYOR
  ✅ Salt okunur 'Dönüştür' düğmesini GÖRMÜYOR
  ✅ Yönetici 'Dönüştür' düğmesini GÖRÜYOR (kontrolün boşa düşmediğini kanıtlar)
  ✅ Salt okunur 'Yeni Teklif' düğmesini GÖRMÜYOR
  ✅ İkinci kiracı kendi adaylarını görüyor
  ✅ İkinci kiracı diğerinin firmasını tekliflerde görmüyor
10. Otomasyon ve iletişim — bildirim, takvim, iş akışı
  ✅ Bildirim merkezi açılıyor
  ✅ Bildirim tercihleri açılıyor
  ✅ Takvim açılıyor
  ✅ Takvim ızgarası gün başlıklarıyla çiziliyor
  ✅ Yönetici otomasyon ekranını açabiliyor
  ✅ Seed'deki örnek kurallar görünüyor
  ✅ E-posta ayar ekranı açılıyor
  ✅ Parolaların şifreli saklandığı ekranda yazıyor
  ✅ Üye otomasyon ekranına giremiyor — http://localhost:3100/yetkisiz?izin=otomasyon.goruntule
  ✅ Üye e-posta ayarlarına giremiyor — http://localhost:3100/yetkisiz?izin=otomasyon.eposta
  ✅ Salt okunur takvimi görebiliyor
  ✅ Zamanlanmış iş uç noktası anahtarsız reddediyor — HTTP 401
11. Veri giriş/çıkış — dışa aktarım, içe aktarım, PDF
  ✅ Listede 'Dışa Aktar' düğmesi var
  ✅ İçe aktarım sihirbazı açılıyor
  ✅ Sihirbaz üç adımı anlatıyor
  ✅ Grup izniyle içe aktarım sayfası açılıyor — http://localhost:3100/ice-aktar
  ✅ İzinli veri kümeleri listeleniyor (Firmalar, Eğitimler) — Firmalar, Eğitimler
  ✅ İzni olmayan kümeler listede YOK (Adaylar, Hizmetler) — Firmalar, Eğitimler
  ✅ CSV dışa aktarım dosya döndürüyor
  ✅ CSV BOM ile başlıyor (Excel Türkçe karakterleri doğru açar)
  ✅ CSV başlıkları noktalı virgülle ayrılmış
  ✅ Excel dışa aktarım geçerli bir .xlsx döndürüyor — ZIP imzası (PK)
  ✅ Tanımsız veri kümesi dışa aktarılamıyor — HTTP 400
  ✅ Salt okunur firma listesini dışa aktarabiliyor
  ✅ Oturumsuz dışa aktarım engelleniyor — HTTP 307 (login'e yönlendirme)
  ✅ Teklif yazdırma sayfası açılıyor
  ✅ Yazdırma sayfasında kuruluş markası var
12. Kişiselleştirme ve süreklilik — pano, görünümler, yedekleme
  ✅ Panoda 'Panoyu Düzenle' düğmesi var
  ✅ Listede 'Görünümler' düğmesi var
  ✅ Yönetici yedek ekranını açabiliyor
  ✅ Ekran geri yüklemenin ekleyici olduğunu anlatıyor
  ✅ Üye yedek ekranına giremiyor — http://localhost:3100/yetkisiz?izin=yedek.yonet
  ✅ Salt okunur yedek ekranına giremiyor — http://localhost:3100/yetkisiz?izin=yedek.yonet
  ✅ Üyenin menüsünde 'Yedekler' bağlantısı yok
  ✅ Oturumsuz yedek indirme engelleniyor — HTTP 307
13. Kiracıya özel alanlar — tanım, form, filtre, izolasyon
  ✅ Yönetici alan tanımlama ekranını açabiliyor
  ✅ Seed'deki örnek alanlar listeleniyor
  ✅ Firma formunda özel alan bölümü var
  ✅ Seçim tipli alan firma listesinde filtre olarak sunuluyor
  ✅ Üye alan tanımlama ekranına giremiyor — http://localhost:3100/yetkisiz?izin=ozelalan.yonet
  ✅ Üye formda özel alanları görüyor (değer girmek varlık iznine tabidir)
  ✅ Komşu kiracının formunda bu alanlar YOK (kiracıya özel)
14. Kullanıcı yönetimi — kuruluş yöneticisinin ekip ekranı
  ✅ Yönetici kullanıcı ekranını açabiliyor
  ✅ Kuruluşun kullanıcıları listeleniyor
  ✅ Komşu kiracının kullanıcısı listede YOK
  ✅ Üye kullanıcı yönetimine giremiyor — http://localhost:3100/yetkisiz?izin=kullanici.yonet
15. Hesap güvenliği ve KVKK — şifre, 2FA, oturum, aydınlatma
  ✅ Şifremi unuttum sayfası oturumsuz açılıyor — HTTP 200
  ✅ Giriş ekranında şifremi unuttum bağlantısı var
  ✅ Sıfırlama formu e-posta istiyor
  ✅ Geçersiz sıfırlama bağlantısı reddediliyor
  ✅ Geçersiz bağlantı hiçbir hesap bilgisi sızdırmıyor
  ✅ Hesap güvenliği ekranı açılıyor
  ✅ Açık oturumlar listeleniyor (bu cihaz işaretli)
  ✅ Şifre değiştirme mevcut şifreyi istiyor
  ✅ Üye de kendi güvenlik ayarlarını açabiliyor — http://localhost:3100/guvenlik
  ✅ KVKK aydınlatma metni açılıyor
  ✅ Metin sürümlü ve rıza isteniyor
  ✅ Veri kopyası indirme sunuluyor
  ✅ Kuruluş güvenlik politikası kullanıcılar ekranında
  ✅ Oturumsuz KVKK veri indirme engelleniyor — HTTP 307
16. Kişi departmanı — sabit listeden aramalı seçim
  ✅ Kişiler listesinde Departman sütunu var
  ✅ Seed verisinde departman dolu
  ✅ Firma detayındaki kişi tablosunda Departman sütunu var
  ✅ Departman sabit listeden seçilir (serbest metin şeması yok)
17. Faz 13 — firma numarası, arama, menü düzeni
  ✅ Firma listesinde numara sütunu var (H1)
  ✅ Firma numarasıyla arama o firmayı buluyor (H1)
  ✅ Büyük harfle arama da aynı firmayı buluyor (H2)
  ✅ CRM çubuğunda "Kontaklar" var (H3)
  ✅ Eski "Kişiler" etiketi hiçbir yerde kullanılmıyor (H3)
  ✅ Menüde ayrı "Adaylar" başlığı YOK (H5)
  ✅ Fırsatlar ekranında Adaylar sekmesi var (H5)
  ✅ Takvimde kategori süzgeci kayıt sayısını daraltıyor (H8)
  ✅ Raporlarda tarih aralığı uygulanıyor (H9)
18. Faz 14 — katalog, paket, kampanya, stok
  ✅ Yönetici ürün kataloğunu açabiliyor
  ✅ Katalogda liste fiyatı ve KDV dahil sütunu var
  ✅ Paket ekranı paket fiyatını ve müşteri avantajını gösteriyor
  ✅ Kampanya listesi kota ve kullanım özetini gösteriyor
  ✅ Stok ekranı bakiyeleri listeliyor
  ✅ Kritik stok uyarısı görünüyor (El Terminali eşiğin altında)
  ✅ Üye katalogu görüntüleyebiliyor
  ✅ Üye 'Yeni Ürün' düğmesini GÖRMÜYOR (tanım yöneticinin işi)
  ✅ Üye kampanyaları görüyor ama 'Yeni Kampanya' düğmesi yok
  ✅ Komşu kiracı Gezegen'in ürünlerini GÖRMÜYOR
19. Faz 15 — sipariş onayı ve sevkiyat kapısı
  ✅ Yönetici sipariş listesini açabiliyor
  ✅ Onay kuyruğu yöneticiye gösteriliyor
  ✅ Onay bekleyen siparişte 'Sevkiyat Aç' düğmesi YOK
  ✅ Sebebi ekranda yazılı (onaylanmadan sevkiyat açılamaz)
  ✅ Onay/ret düğmeleri yöneticide var
  ✅ Onaylanmış siparişte 'Sevkiyat Aç' düğmesi VAR
  ✅ Ret gerekçesi sipariş detayında görünüyor
  ✅ Üye sipariş listesini görüyor
  ✅ Üyede onay kuyruğu YOK (onay yöneticinin işi)
  ✅ Üye onay/ret düğmelerini GÖRMÜYOR
  ✅ Sevkiyat ekranı açılıyor ve kuyruğu gösteriyor
  ✅ Komşu kiracı Gezegen'in siparişlerini GÖRMÜYOR
20. Faz 16 — proje, destek kaydı ve bilgi bankası
  ✅ Yönetici proje listesini açabiliyor
  ✅ Proje detayında bağlı destek kayıtları bölümü var
  ✅ Proje bütçesi görünüyor
  ✅ Destek listesi açılıyor ve kayıt numarası taşıyor
  ✅ Varsayılan görünüm AÇIK işlerdir (kapanmış kayıt listeyi doldurmaz)
  ✅ Kapanmış kayıt varsayılan listede YOK
  ✅ 'Hepsi' süzgeci kapanmış kaydı getiriyor
  ✅ Kapanmış kayıtta çözüm süresi hesaplanmış
  ✅ Destek raporu açılıyor
  ✅ SSS ekranı açılıyor
  ✅ Yönetici SSS içeriğini yönetebiliyor
  ✅ SSS araması Türkçe büyük/küçük harften bağımsız
  ✅ Üye SSS'yi okuyor ama 'Yeni Soru' düğmesini GÖRMÜYOR
  ✅ Komşu kiracı Gezegen'in destek kayıtlarını GÖRMÜYOR
21. Faz 17 — dosya eki ve saha ziyareti
  ✅ Ziyaret ekranı açılıyor
  ✅ Kuruluşun yarıçap ayarı ekranda yazılı
  ✅ Doğrulanmış ziyaret rozeti var
  ✅ Uyuşmayan konum rozeti var
  ✅ Konumu alınamayan ziyaret 'uzak' DEĞİL, 'doğrulanamadı'
  ✅ Konum süzgeci yalnızca uyuşmayanları getiriyor
  ✅ Firma detayında konum görünüyor
  ✅ Firma detayında ek bölümü var
  ✅ Ek sınırı kullanıcıya yazılı (10 MB)
  ✅ Üye ziyaret ekranını açabiliyor (saha işi üyenindir)
  ✅ Dosya ucu oturumsuz erişime kapalı — HTTP 307
  ✅ Komşu kiracı Gezegen'in ziyaretlerini GÖRMÜYOR
22. Faz 18 — rapor merkezi ve firma dosyası
  ✅ Rapor merkezi açılıyor
  ✅ Mali rapor kartı listeleniyor
  ✅ Modülün kendi raporu merkeze BAĞLANMIŞ (kopyalanmamış)
  ✅ Mali rapor açılıyor
  ✅ Tahsilat rakamının TAHMİN olduğu ekranda yazılı
  ✅ Satış hattı raporu açılıyor
  ✅ Aktivite yükü raporu açılıyor
  ✅ Ürün satış raporu açılıyor
  ✅ Genel durum raporu yeni adresinde
  ✅ Firma dosyası açılıyor
  ✅ Dosyada künye bölümü var
  ✅ Dosyada firmanın adı geçiyor
  ✅ Salt okunur kullanıcı firma dosyasını açabiliyor
  ✅ Üye rapor merkezini açabiliyor
23. Faz 19 — anket ve anonim yanıt toplama
  ✅ Anket listesi açılıyor
  ✅ Anonim anket listede işaretli
  ✅ Anonim ankette 'ne yanıtladığı bilinmez' uyarısı var
  ✅ Anket raporu NPS gösteriyor
  ✅ Anonim raporda firma kırılımı ÜRETİLEMEZ diye yazıyor
  ✅ Anonim ankette yanıtlar kişiye/firmaya BAĞLI DEĞİL (veride) — kimlikli yanıt: 0
  ✅ Kimlikli ankette yanıtlar gönderime BAĞLI — bağlı yanıt: 12
  ✅ Anket sayfası OTURUMSUZ açılıyor (giriş istemiyor) — http://localhost:3100/anket/gecersiz-token
  ✅ Geçersiz token hiçbir bilgi sızdırmıyor
  ✅ Üye anket listesini görüyor
  ✅ Üye 'Yeni Anket' düğmesini GÖRMÜYOR (tanım yöneticinin işi)
  ✅ Komşu kiracı Gezegen'in anketlerini GÖRMÜYOR
  ✅ Üst çubukta arama/komut paleti tetikleyicisi var
  ✅ Arama ucu oturumsuz erişime kapalı — HTTP 307
  ✅ Özet ucu oturumsuz erişime kapalı — HTTP 307
  ✅ Firma ekranında sekme çubuğu var
  ✅ Genel sekmesi zaman akışını gösteriyor
  ✅ Genel sekmesinde satış tabloları ÇİZİLMİYOR (sekme bir sorgu kapısıdır)
  ✅ Satış sekmesi teklifleri gösteriyor
  ✅ Satış sekmesi siparişleri gösteriyor
  ✅ Proje & Destek sekmesi açılıyor
  ✅ Uydurma sekme hata vermeden Genel'e düşüyor
  ✅ Yan panel querystring'den açılıyor
  ✅ Komşu kiracı Gezegen firmasının özetini AÇAMIYOR
  ✅ AI ayar ekranı açılıyor
  ✅ Modele NE gönderildiği ekranda yazılı
  ✅ Modele NE GÖNDERİLMEDİĞİ de yazılı
  ✅ AI varsayılan KAPALI (kiracı bilerek açar)
  ✅ Skorun dış çağrı yapmadığı kullanıcıya söyleniyor
  ✅ Fırsat listesinde skor sütunu var (AI kapalıyken de)
  ✅ Aday listesinde skor sütunu var
  ✅ Firma ekranında özet paneli var
  ✅ Özet AI kapalıyken de içerik gösteriyor
  ✅ Salt okunur kullanıcı AI ekranına giremiyor
  ✅ Üye AI ekranını görüyor
  ✅ Üye AI ayarını DEĞİŞTİREMİYOR (kuruluş kararı)
  ✅ Üye kullanım defterini GÖRMÜYOR
  ✅ Sol menüde CRM bölümü var
  ✅ Sol menüde Satış Yönetimi bölümü var
  ✅ SSS sol menüde 'Bilgi Bankası' etiketiyle duruyor
  ✅ CRM sekme çubuğu Firmalar ekranında çiziliyor
  ✅ Satış sekme çubuğu Teklifler ekranında çiziliyor
  ✅ Takvimde bölüm sekme çubuğu YOK
  ✅ Ayarlar menüde Yönetim bölümünde
  ✅ Eski adres çalışmaya devam ediyor: /kisiler
  ✅ Eski adres çalışmaya devam ediyor: /urunler
  ✅ Eski adres çalışmaya devam ediyor: /kampanyalar
  ✅ Eski adres çalışmaya devam ediyor: /hizmetler
  ✅ Üye CRM sekme çubuğunu görüyor
  ✅ Sipariş formunda kampanya alanı görünüyor
  ✅ Kampanya yoksa SEBEBİ yazıyor (sessizce gizlenmiyor)
  ✅ Teklif formunda ürün seçici var (katalog bağı)
  ✅ Teklif formunda kampanya alanı var
  ✅ Raporda PDF düğmesi var: /raporlar/mali
  ✅ Raporda PDF düğmesi var: /raporlar/satis
  ✅ Raporda PDF düğmesi var: /raporlar/urun
  ✅ Raporda PDF düğmesi var: /raporlar/aktivite
  ✅ Raporda PDF düğmesi var: /raporlar/genel
  ✅ Destek raporunda da PDF düğmesi var
  ✅ Sipariş formunda 'Paketten kalem ekle' seçicisi var
  ✅ Teklif formunda 'Paketten kalem ekle' seçicisi var
  ✅ Seed'deki genel paket sipariş formunda LİSTELENİYOR
  ✅ Seed'deki genel paket teklif formunda LİSTELENİYOR
  ✅ Üye de paket seçicisini görüyor (katalog izniyle gelir)
  ✅ Başka kiracının paketi sipariş formunda görünmüyor
  ✅ Kampanya kapsamında satırların SOLUNDA onay kutusu var
  ✅ Kapsam artık `select multiple` DEĞİL
  ✅ Kapsamın üstünde 'kaç tane seçili' bilgisi yazıyor
  ✅ Kapsamda 'Tümünü seç' düğmesi var
  ✅ 'Tümünü seç' gerçekten hepsini işaretliyor
  ✅ Seçim durumu değişti (düğme etkisiz değil)
  ✅ İçeriden sürükleyip dışarıda bırakmak modalı KAPATMIYOR
  ✅ Dışarı KASITLI tıklamak modalı kapatıyor
  ✅ Sol menüde 'Ayarlar' girişi var
  ✅ Ayarlar sekme çubuğunda: Kullanıcılar
  ✅ Ayarlar sekme çubuğunda: Gruplar
  ✅ Ayarlar sekme çubuğunda: Özel Alanlar
  ✅ Ayarlar sekme çubuğunda: Otomasyon
  ✅ Ayarlar sekme çubuğunda: E-posta
  ✅ Ayarlar sekme çubuğunda: AI Özellikleri
  ✅ Ayarlar sekme çubuğunda: Yedekler
  ✅ Ayarlar sekme çubuğunda: İçe Aktar
  ✅ Ayarlar sekmesi eski adresinde açılıyor: /yedekler
  ✅ Ayarlar sekmesi eski adresinde açılıyor: /ai
  ✅ Ayarlar sekmesi eski adresinde açılıyor: /otomasyon/eposta
  ✅ Ayarlar sekmesi eski adresinde açılıyor: /ozel-alanlar
  ✅ Denetim Günlüğü Ayarlar'a alınmadı (ayar değil, kayıttır)
  ✅ Cam katmanları sayfada çiziliyor — 25 katman
  ✅ SVG kırılma filtresi kabukta tanımlı
  ✅ Cam katmanları TIKLAMAYI ENGELLEMİYOR
  ✅ Cam sekme çubuğundan gezinme çalışıyor — http://localhost:3100/kisiler
  ✅ Cam token'ları koyu ve açık temada FARKLI — rgba(255,255,255,0.34) / rgba(146,168,214,0.11)
```

